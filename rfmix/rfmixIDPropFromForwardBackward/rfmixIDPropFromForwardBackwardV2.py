...
"""
    ...
    Usage:
        rfmixIDPropFromForwardBackwardV2.py -n nancestry [-s|(-i --ms file)]
        rfmixIDPropFromForwardBackwardV2.py -h | --help | -v | --version | -f | --format
...
if __name__ == '__main__':
    ...
    if args['-s']:
        ...
        for i in range(0, len(ancMean), nanc):
            out = []
            for j in range(nanc):
                out.append(ancMean[i+j])
            for j in range(nanc):
                out.append(ancSTD[i+j])
